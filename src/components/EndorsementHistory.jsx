...
/**
 * @param {{ scheduleExperienceVersion?: 'v1' | 'v2' | 'v3' | 'v4' }} props — V1/V2: tabbed Endorsements + schedules (formerly internal v3).
 */
export default function EndorsementHistory({ scheduleExperienceVersion = 'v1' }) {
  const { history, updateEntry } = useEndorsements()
  const isTabbedLayout =
    scheduleExperienceVersion === 'v1' ||
    scheduleExperienceVersion === 'v2' ||
    scheduleExperienceVersion === 'v3'
  const isV4 = scheduleExperienceVersion === 'v4'
  const showEndorsementsScheduleChips = false
  ...
  const showEndorsementsTable = !isTabbedLayout || v3MainTab === 'endorsements'
    ...
    <>
      <div className="bg-white rounded-xl border border-gray-200 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              <div
                className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto [min-height:max(16rem,28dvh)]"
                data-testid="endorsement-history-scroller"
              >
          <table className="w-full min-w-0 table-fixed border-collapse">
            ...
            <thead className="sticky top-0 z-[1]">
              <tr className={ENDORSEMENT_THEAD_TR_CLASS}>
                ...
                <EndorsementSortTh
                  ...
                  className={isTabbedLayout ? ENDORSEMENT_TABLE_HEAD_EDGE_PR : undefined}
                >
                  Schedule status
                </EndorsementSortTh>
                {!isTabbedLayout ? (
            ...
            <tbody className="divide-y divide-gray-100">
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={isTabbedLayout ? 8 : 9} className={`${ENDORSEMENT_TABLE_EDGE_PL} ${ENDORSEMENT_TABLE_EDGE_PR} py-14 text-center align-middle`}>
                    {history.length === 0 ? (
              ...
              paginated.map((row) => (
                ...
                >
                  ...
                  <td className={`relative overflow-visible align-middle ${isTabbedLayout ? ENDORSEMENT_TABLE_CELL_EDGE_PR : ENDORSEMENT_TABLE_CELL_INNER}`}>
                    <EndorsementScheduleStatusCell
                      row={row}
                      showPendingCoachmark={isTabbedLayout && v3MainTab === 'endorsements'}
                      onGoToSchedules={isTabbedLayout ? openV3SchedulesTab : undefined}
                    />
                  </td>
                  {!isTabbedLayout ? (
        ...
        {isTabbedLayout && v3MainTab === 'schedules' ? (
